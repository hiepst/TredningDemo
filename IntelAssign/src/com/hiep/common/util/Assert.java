...
public class Assert {
	...
	/**
	 * Assert a boolean expression associatd with an object, throwing a
	 ...
	 * @param message
	 *            The exception message to display if the expression is
	 *            <code>false</code>
	 * @param value
	...
	/**
	 * Assert a boolean expression associated with a decimal primitive, throwing
	 * <code>IllegalArgumentException</code> if the test result is
	 * <code>false</code>.
	 *
	 * @param expression
	 ...
	 * Assert a boolean expression, throwing
	 * <code>IllegalArgumentException</code> if the test result is
	 * <code>false</code>. The exception message is a generic, default message.
	 *
	 * @param expression
	 *            A boolean expression
	 * @throws IllegalArgumentException
	...
	/**
	 * Assert that an object is not null. If the object is null, a
	 * <code>IllegalArgumentException</code> is thrown and the default exception
	 * message is used.
	 * <p>
	 *
	 * <pre>
	...
	/**
	 * Assert that an object array has elements; that is, it must not be
	 * <code>null</code> and must have a <code>length > 0</code>.
	 * <p>
	 *
	 * <pre>
	 ...
	 * @throws IllegalArgumentException
	 *             If the object array is <code>null</code> or has a
	 *             <code>length > 0</code>
	 */
	...
	/**
	 * Assert that an object array has elements; that is, it must not be
	 * <code>null</code> and must have a <code>length > 0</code>. If the
	 * assertion fails, an <code>IllegalArgumentException</code> is thrown and
	 * the default exception message is used.
	 * <p>
	 *
	 * <pre>
	 ...
	 * <code>null</code> and must have a <code>size > 0</code>.
	 * <p>
	 *
	 * <pre>
	 ...
	 *
	 * @param collection
	 *            The collection to check to see if it has entries.
	 * @param message
	 *            The exception message to use if the assertion fails.
	 * @throws IllegalArgumentException
	 *             If the collection is <code>null</code> or has a
	 *             <code>size > 0</code>
	 */
	...
	/**
	 * Assert that a Map has entries; that is, it must not be <code>null</code>
	 * and must have at least one <code>key-value pair</code>. If the assertion
	 * fails, an <code>IllegalArgumentException</code> is thrown and the default
	...
	public static void hasEntries(Map map) {
		if (map == null || map.isEmpty()) {
			throw new IllegalArgumentException(
					"Assertion failed; this map must have at least one entry (key-value pair.)");
		}
	}
	...
	public static void isInRange(int number, int low, int high) {
		if (number < low || number > high) {
			throw new IllegalArgumentException(
					"Assertion failed; " + number + " is not in range [" + low + ", " + high + "]");
		}
	}
	 ...
	 *             If number is out of range.
	 */
	public static void isInRange(long number, long low, long high) {
		if (number < low || number > high) {
			throw new IllegalArgumentException(
					"Assertion failed; " + number + " is not in range [" + low + ", " + high + "]");
		}
	 ...
	 * @param text
	 *            The text to be matched against
	 * @throws IllegalArgumentException
	 *             If the text does not match the regex pattern
	 */
	public static void matches(String regex, CharSequence text) {
		if (!Pattern.matches(regex, text)) {
			throw new IllegalArgumentException("Assertion failed; " + text + " does not match pattern " + regex);
		}
	}
