 ...
 *
 * <P>
 * Intended for use with <tt>Action</tt> and <tt>JMenuItem</tt>. Alignment of
 * text is poor when the same menu mixes menu items without an icon with menu
 * items having an icon. In such cases, items without an icon can use an
 * <tt>EmptyIcon</tt> to take up the proper amount of space, and allow for
 * alignment of all text in the menu.
 */
