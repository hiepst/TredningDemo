...
import com.cs.client.util.Args;
import com.cs.client.util.Consts;
import com.cs.client.util.Util;
...
public final class UiUtil {
	 ...
	 *
	 * <P>
	 * If the size of <tt>aWindow</tt> exceeds that of the screen, then the size
	 * of <tt>aWindow</tt> is reset to the size of the screen.
	 */
	 ...
	 *
	 * @param aWindow
	 *            must have non-null result for <tt>aWindow.getParent</tt>.
	 */
	 ...
	 * 
	 * @param aButtons
	 *            contains the buttons to be placed in a column
	 */
	 ...
	 *
	 * @param aImageId
	 *            starts with '/', and refers to an image resource which is
	 ...
	 *
	 * @param aImageId
	 *            does NOT start with '/', and must refer to an image resource
	 *            which is accessible through {@link Class#getResource}.
	 * @param aClass
	 *            the class relative to which the image is located.
	 */
	...
	/**
	 * Return a square icon which paints nothing, and whose dimensions
	 * correspond to the user preference for icon size.
	 *
	 * <P>
	 * A common problem occurs with text alignment in menus, where there is a
	 * mixture of menu items with and without an icon. Adding an empty icon to
	 * menu items which do not have one will adjust its alignment to match that
	 * of the others which do have an icon.
	 */
	public static Icon getEmptyIcon() {
		// GeneralLookPreferencesEditor prefs = new
	 ...
	 *
	 * @param aPercentWidth
	 *            percentage width of the screen, in range <tt>1..100</tt>.
	 * @param aPercentHeight
	 *            percentage height of the screen, in range <tt>1..100</tt>.
	 */
	 ...
	 * strings and customized fonts. (See this
	 * <a href="http://java.sun.com/products/jlf/ed2/samcode/prefere.html">Sun
	 ...
	 *
	 * <P>
	 * The <tt>JLabel</tt> appears on the left, and the <tt>JTextField</tt>
	 * appears on the same row, just to the right of the <tt>JLabel</tt>. The
	 * <tt>JLabel</tt> has a mnemonic which forwards focus to the
	 * <tt>JTextField</tt> when activated.
	 * 
	 * @param aContainer
	 *            holds the pair of components.
	 * @param aName
	 *            text of the <tt>JLabel</tt> component.
	 * @param aInitialValue
	 *            possibly-null initial value to appear in the
	 *            <tt>JTextField</tt>; if <tt>null</tt>, then
	 *            <tt>JTextField</tt> will be blank.
	 * @param aMnemonic
	 *            <tt>KeyEvent</tt> field, used as the mnemonic for the
	 *            <tt>JLabel</tt>.
	 * @param aConstraints
	 *            applied to the <tt>JLabel</tt>; the corresponding constraints
	 *            for the <tt>JTextField</tt> are the same as
	 *            <tt>aConstraints</tt>, except for <tt>gridx</tt> being
	 *            incremented by one; in addition, if <tt>aConstraints</tt> has
	 *            <tt>weightx=0</tt> (the default), then the entry field will
	 *            receive <tt>weightx=1.0</tt> (entry field gets more horizontal
	 *            space upon resize).
	 * @param aTooltip
	 *            possibly-null text displayed as tool tip for the
	 *            <tt>JTextField</tt> ; if <tt>null</tt>, the tool tip is turned
	 *            off.
	...
	/**
	 * Create a pair of <tt>JLabel</tt> components, as is typically needed for
	 ...
	 * {@link UiConsts#MAX_LABEL_LENGTH}, then the text is truncated, an
	 * ellipsis is placed at its end, and the full text is placed in a tooltip.
	 * 
	 * @param aContainer
	 *            holds the pair of components.
	 * @param aName
	 *            text of the name <tt>JLabel</tt>.
	 * @param aValue
	 *            possibly-null ; if null, then an empty <tt>String</tt> is used
	 *            for the value; otherwise <tt>Object.toString</tt> is used.
	 * @param aConstraints
	 *            for the name <tt>JLabel</tt>; the corresponding constraints
	 *            for the value <tt>JLabel</tt> are mostly taken from
	 *            <tt>aConstraints</tt>, except for <tt>gridx</tt> being
	 *            incremented by one (<tt>weightx</tt> may differ as well - see
	 *            <tt>aWeightOnDisplay</tt>.)
	 * @param aWeightOnDisplay
	 *            if true, then set <tt>weightx</tt> for the value field to 1.0
	 *            (to give it more horizontal space upon resize).
	 ...
	 * {@link #getConstraints(int, int)}, and its <tt>aWeightOnDisplay</tt>
	 ...
	 *
	 * <P>
	 * The number of items which should be presented using this method is
	 * limited, since no scrolling mechanism is given to the user.
	 *
	 * @param aContainer
	 *            holds the display fields.
	 * @param aNameValuePairs
	 *            has <tt>String</tt> keys for the names, and values are
	 *            possibly null <tt>Object</tt>s; if null, then an empty
	 *            <tt>String</tt> is displayed, otherwise
	 ...
	 * bottom row of a <tt>GridBagLayout</tt> of components. When resized, this
	 * glue will take up extra vertical space.
	 *
	 * <P>
	 ...
	 *
	 * @param aPanel
	 *            uses <tt>GridBagLayout</tt>, and contains components whose
	 ...
	 *
	 * @param aText
	 *            must have visible content, doesn't contain newline characters
	 *            or html.
	 * @return <tt>JTextArea</tt> which is not editable, has improved spacing
	 *         over the supplied default (placing {@link UiConsts#ONE_SPACE} on
	 *         the left and right), which wraps lines on word boundaries, and
	 *         whose background color is the same as
	 ...
	 * <tt>aText</tt>, and not by the widget.
	 *
	 * @param aText
	 *            has visible content
	 * @return <tt>JTextArea</tt> which is not editable, has improved spacing
	 *         over the supplied default (placing {@link UiConsts#ONE_SPACE} on
	 *         the left and right), and whose background color is the same as
	 *         {@link javax.swing.plaf.metal.MetalLookAndFeel#getMenuBackground}
	 *         .
	 */
	 ...
	 *
	 * <P>
	 * Intended especially for <tt>BoxLayout</tt>, where all components need to
	 * share the same alignment in order for display to be reasonable. (Indeed,
	 * this method may only work for <tt>BoxLayout</tt>, since apparently it is
	 * the only layout to use <tt>setAlignmentX, setAlignmentY</tt>.)
	 *
	 * @param aContainer
	 *            contains only <tt>JComponent</tt> objects.
	 */
	...
	/**
	 * If <tt>aIconName</tt> indicates that the icon is part of the standard
	 * graphic repository (by starting with "/toolbar"), then append either
	 * "16.gif" or "24.gif" to the name, according to the user's current
	 * preference for icon size.
	 */
